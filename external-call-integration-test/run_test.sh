...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help|-h)
      ...
      echo "  CANTON_CMD    Path to Canton binary (use for custom Canton with external call support)"
      echo ""
      echo "Test suites:"
...
# ============================================================
# Find Canton executable
# ============================================================
echo "[1/5] Looking for Canton executable..."

# Check for CANTON_CMD environment variable first (allows custom canton binary)
if [ -n "${CANTON_CMD:-}" ] && [ -x "$CANTON_CMD" ]; then
  echo "       Using CANTON_CMD from environment: $CANTON_CMD"
else
  # Default to Canton from the daml SDK
  ...
  if [ ! -x "$CANTON_CMD" ]; then
    ...
    echo ""
    echo "Or set CANTON_CMD to a custom Canton binary with external call support."
    exit 1
  fi
