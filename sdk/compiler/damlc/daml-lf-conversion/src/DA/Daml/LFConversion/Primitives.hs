...
convertPrim _ "BEExternalCall" (TText :-> TText :-> TText :-> inputTy :-> TUpdate outputTy) =
    let v4 = mkVar "v4"
    in pure $ ETmLam (varV1, TText) $ ETmLam (varV2, TText) $ ETmLam (varV3, TText) $ ETmLam (v4, inputTy) $
        EUpdate $ UEmbedExpr outputTy $
            EBuiltinFun BEExternalCall `ETyApp` inputTy `ETyApp` outputTy `ETmApp` EVar varV1 `ETmApp` EVar varV2 `ETmApp` EVar varV3 `ETmApp` EVar v4
