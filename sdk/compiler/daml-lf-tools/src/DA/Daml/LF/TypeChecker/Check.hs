...
typeOfBuiltin = \case
  ...
  BESha256Hex        -> pure $ TText :-> TText
  BEKecCak256Text    -> pure $ TText :-> TText
  BEEncodeHex        -> pure $ TText :-> TText
  BEDecodeHex        -> pure $ TText :-> TText
  BEExternalCall     -> checkFeature featureExternalCall >> pure (TText :-> TText :-> TText :-> TText :-> TUpdate TText)
...
checkIface m iface = do
  ...
  where
    tcon = Qualified SelfPackageId (moduleName m) (intName iface)
...
checkTemplate m t@(Template _loc tpl param precond signatories observers choices mbKey implements) = do
  let tcon = Qualified SelfPackageId (moduleName m) tpl
...
checkDefException m DefException{..} = do
    let modName = moduleName m
        tcon = Qualified SelfPackageId modName exnName
